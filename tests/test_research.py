...
def test_purge_must_cover_label_horizon() -> None:
    with pytest.raises(ValueError, match="at least label_horizon"):
        expanding_walk_forward(100, min_train_size=40, test_size=10, purge_size=4, label_horizon=5)


def test_walk_forward_is_ordered_and_purged() -> None:
    splits = expanding_walk_forward(30, min_train_size=10, test_size=5, purge_size=2, label_horizon=2)
    assert len(splits) == 3
...
def test_logistic_baseline_returns_fold_metrics() -> None:
    ...
    splits = expanding_walk_forward(80, min_train_size=50, test_size=10, purge_size=1, label_horizon=1)
    metrics = evaluate_logistic_baseline(frame, feature_columns=["x"], label_column="y", splits=splits)
